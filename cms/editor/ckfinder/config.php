...
require_once(ABS_PATH.'./config/config.inc.php');
include ABS_PATH.'./'.MANAGE_DIR.'/base.php';

$session_storage = getSessionStorageType();
if($session_storage=='session_mysql'){
	include_once(ABS_PATH.DIRECTORY_SEPARATOR.MANAGE_DIR.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'session_mysql.class.php');
	$sessionMySql=new session_mysql();
}elseif($session_storage=='session_files'){
	include_once(ABS_PATH.DIRECTORY_SEPARATOR.MANAGE_DIR.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'session_files.class.php');
	$sessionFiles=new session_files();
}else {
	session_start();
}
function CheckAuthentication()
{
	...
	// forget to add session _start() at the top of this file.
...
if (isset($_SESSION['cms_dir_path'])&&strlen($_SESSION['cms_dir_path'])){
	$baseUrl = $_SESSION['cms_dir_path'].'/upload/';
}else {
	$baseUrl = 'upload/';
}
...
if (isset($_SESSION['url'])&&strlen($_SESSION['url'])){
	$baseDir = ABS_PATH.$_SESSION['siteDir'].DIRECTORY_SEPARATOR.'upload'.DIRECTORY_SEPARATOR;
}else {
	$baseDir = ABS_PATH.'upload'.DIRECTORY_SEPARATOR;
}
/*
 * ### Advanced Settings
 */
...
$cmsContentConfigFilePath=ABS_PATH.'config'.DIRECTORY_SEPARATOR.'cmsContent.config.php';
$cmsContentConfig = include $cmsContentConfigFilePath;
$maxPicWidth=$cmsContentConfig['maxPicWidth'];
//personnal end
$config['Images'] = Array(
		'maxWidth' => intval($maxPicWidth),
		'maxHeight' => 1200,
		'quality' => 80);
...
uncommenting the following "session _start()" call.
*/
