CKEDITOR.plugins.add( 'autoformat',
{
	init : function( editor )
	{
		editor.addCommand( 'autoformat', new CKEDITOR.autoformatCommand());
			editor.ui.addButton('autoformat',{label:"自动排版",command:'autoformat',icon:this.path + 'images/autoformat.gif'});
	}
});
...
function FormatText(editor) {
   ...
   if (myeditor.mode=="wysiwyg"){
        ...
        if (!isPart) {
           ...
           var html = processFormatText(tmpDiv.innerText);//自己注释
		  // var html = tmpDiv.innerText;//自己写
