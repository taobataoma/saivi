CKEDITOR.plugins.add( 'iframedialog',
{
	...
	onLoad : function()
	{
		...
		* If it isn't specified, the inner frame is notified of the dialog events ('load', 'ok' and 'cancel')
		* on a function called 'onDialogEvent'
		* @param {Object} [userDefinition] Additional properties for the dialog definition
		* @example
		*/
		CKEDITOR.dialog.addIframe = function( name, title, src, width, height, onContentLoad, userDefinition )
		{
			...
			element.onContentLoad = function()
			{
				...
				if ( childWindow.onDialogEvent )
				{
					...
					notifyEvent = function(e)
					{
						/*******self start*******/
						if(e.name=='ok'){
							var returnStr=childWindow.onDialogEvent(e);
							editor.insertHtml(returnStr);
						}else{
							return childWindow.onDialogEvent(e);
						}
						/*******self end*******/
						//return childWindow.onDialogEvent(e);//original
					};
