...
final class mysql {
	...
	public function connect() {
		...
		if(!$this->link = @$func($this->config['hostname'].':'.$this->config['port'], $this->config['username'], $this->config['password'], 1)) {
	 ...
	 * 						为字符串时[例：`name`='',`hits`=`hits`+1]。
	 *						为数组时[例: array('name'=>'','password'=>'123456')]
