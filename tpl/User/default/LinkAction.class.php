...
class LinkAction extends UserAction{
	...
	public function keywordModules(){
		...
		//添加功能库 BY H 20140901
		array('module'=>'Xcm','linkcode'=>'','name'=>'神经猫游戏','sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'熊出没','askeyword'=>1),
		array('module'=>'Dxs','linkcode'=>'','name'=>'熊出没','sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'读心术','askeyword'=>1),
		array('module'=>'Game','linkcode'=>'','name'=>'2048小游戏','sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'2048','askeyword'=>1),
...
//新功能库添加结束
	...
	public function modules(){
		...
		//添加功能库
		array('module'=>'Xcm','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Xcm&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Xcm'],'sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'','askeyword'=>0),
		array('module'=>'Dxs','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Xcm&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Dxs'],'sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'','askeyword'=>0),
		array('module'=>'Invites','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Invites&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Invites'],'sub'=>1,'canselected'=>0,'linkurl'=>'','keyword'=>'','askeyword'=>0),
		array('module'=>'Game','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Game&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Game'],'Game'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'','askeyword'=>0),
		array('module'=>'Baoming','linkcode'=>'{siteUrl}/index.php?g=Wap&m=Baoming&a=index&token='.$this->token.'&wecha_id={wechat_id}','name'=>$this->modules['Baoming'],'Baoming'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'','askeyword'=>0),
...
//20140901功能库添加介绍
	...
	public function Invites(){
		$this->assign('moduleName',$this->modules['Invites']);
		$db=M('Invites');
		$where=$this->where;
		$count      = $db->where($where)->count();
		$Page       = new Page($count,5);
		...
		if ($list){
			foreach ($list as $item){
				array_push($items,array('id'=>$item['id'],'name'=>$item['title'],'linkcode'=>'{siteUrl}/index.php?g=Wap&m=Invites&a=index&token='.$this->token.'&wecha_id={wechat_id}&classid='.$item['id'],'linkurl'=>'','keyword'=>$item['keyword']));
			}
		}
		//
		$this->assign('list',$items);
	...
	//微邀请结束
		...
		$this->assign('moduleName',$this->modules['Baoming']);
		$db=M('Baoming');
		$where=$this->where;
		$count      = $db->where($where)->count();
		$Page       = new Page($count,5);
		...
		if ($list){
			foreach ($list as $item){
				array_push($items,array('id'=>$item['id'],'name'=>$item['title'],'linkcode'=>'{siteUrl}/index.php?g=Wap&m=Baoming&a=index&token='.$this->token.'&wecha_id={wechat_id}&classid='.$item['id'],'linkurl'=>'','keyword'=>$item['keyword']));
			}
		}
		//
		$this->assign('list',$items);
...
//外链小工具
	...
	public function OutsideLink(){
		//处理小工具数组文件
			$f = include('./Saivi/Lib/ORG/Func.links.php');
		//取出分类总汇
			$i=0;
