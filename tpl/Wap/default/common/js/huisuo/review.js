...
//                msg: '鍝庡憖~鍑轰簡鐐瑰効灏忛棶棰橈紝鐐瑰嚮鍒锋柊璇曡瘯鍚'
//            });
//        }
        var data = {
...
//        });
    ...
    reviewResult: function(res) {
        ...
        if (res.ret == 0) {
            ...
            var msg = '鍝庡憖~鍑轰簡鐐瑰効灏忛棶棰橈紝鐐瑰嚮鍒锋柊璇曡瘯鍚';
            if (res.ret == -100) {
                msg = '鍝庡憖~鍑轰簡鐐瑰効灏忛棶棰�';
            }
            REVIEW.showTips(true, {
...
//            });
    ...
    sendReviewResult: function(res) {
        ...
        if (res.ret == 0) {
            ...
            if (count < 1) {
                $('#userReview').html('<span>鍗拌薄寰堢嫭鐗瑰摝</span>');
            } else {
                $('#userReview').html('<em>鎴戠殑妤肩洏鍗拌薄<i>' + R.userReviewText.substr(0, 4) + '</i>涓�' + count + '%鎴垮弸鐩稿悓</em>');
            }
        } else {
            var msg = '鍝庡憖~鍑轰簡鐐瑰効灏忛棶棰橈紝鐐瑰嚮鍒锋柊璇曡瘯鍚';
            if (res.ret == -100) {
