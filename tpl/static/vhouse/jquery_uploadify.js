...
(function($) {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					width           : 100,                // The width of the browse button
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '其中有些文件无法上传:';
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n选中的文件数量超过了文件数量上限 (' + errorMsg + ').';
						} else {
							this.queueData.errorMsg += '\n选中的文件数量超过了文件数量上限  (' + settings.queueSizeLimit + ').';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了尺寸上限 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 是空的.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 不是允许的文件格式 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				$('#' + file.id).find('.data').html(' -上传完成 正在加载.....');
			}
