...
K.options = {
	...
	htmlTags : {
		...
		pre : ['id', 'class'],
		audio : ['id', 'class','src','controls','loop','preload','autoplay'],
...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) { 
	 ...
	 insertmusic :function(url, autoplay) {  
		var html = '<audio  src="' + _escape(url) + '" controls="controls" ';
	    if (autoplay==1) {
...
_plugin('core', function(K) {
	...
	self.plugin.getSelectedAudio = function() {
		return self.cmd.commonAncestor('audio');
	};
