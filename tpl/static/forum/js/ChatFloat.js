var SocketIp = "kf.Saivi.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.Saivi.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.Saiviid && obj.Saiviid.length > 0) {
                    t.initialize(options, obj.Saiviid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"Saiviid":"' + obj.Saiviid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, Saiviid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&Saiviid=' + Saiviid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, Saiviid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, Saiviid);
    },
    GetPolling: function (AId, Saiviid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&Saiviid=' + Saiviid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.Saiviid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.Saiviid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.Saiviid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, Saiviid, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, Saiviid); });
        }
    ...
    socketconnect: function (options, Saiviid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, Saiviid);
                        }
                    }
                });
            });
