...
class Wxhttpclient {
    ...
    public $cookie_host = '';
    public $charset = null;
    function Wxhttpclient($host, $port=80) {
        $this->host = $host;
        $this->port = $port;
    }
        ...
        //判断页面编码
      	...
      	//解决重定向的问题
    	    if($this->status=='301'||$this->status=='302' || isset( $this->headers['location']) && trim( $this->headers['location']) != ''){
    	    	...
    	    	$this->content = Wxhttpclient::quickGet($mburl,30,$this->cookies);    	    	
    	    	return true;
    	    }
    ...
    function getContent($native = false) {
    	...
    	if($this->charset === null){
    		$encode = Wxstring::find_first_string_by_reg(strtolower($this->content),"/\bcharset\s*=\s*\w+[-]?[8]?\b/");
    		if($encode ==''){
    			$encode = Wxstring::find_first_string_by_reg(strtolower($this->content),"/\bcharset\s*=\s*\"\s*\w+[-]?[8]?\s*\"\s*\b/");
    		}
    		if($encode!=''){
    ...
    static function quickGet($url,$time = 30, $justhtml = true, $cks=array()) {
        ...
        $client = new Wxhttpclient($host, $port);
        $client->oldurl = $url;
