...
class AlipayAction extends BaseAction{
	...
	public function pay(){
		...
		if ($alipayConfig['paytype']=='Weixin'){
			header('Location:/index.php?g=Wap&m='.$alipayConfig['paytype'].'&a=pay&price='.$price.'&orderName='.$orderName.'&single_orderid='.$orderid.'&showwxpaytitle=1&from='.$from.'&token='.$this->token.'&wecha_id='.$this->wecha_id);
		}else {
