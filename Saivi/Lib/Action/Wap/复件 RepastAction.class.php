...
*/      
        $categorys = $data->distinct(true)->field('category')->select();
        $this->assign('categorys',$categorys);
        $scope = $data->distinct(true)->field('scope')->select();
    ...
    public function selectTable(){//to Repast/saveUser
        ...
        $dishCompany = M('DishCompany');
        $phone_authorize = $dishCompany->where(array('cid'=>$this->_get('cid')))->field('phone_authorize')->find();
        $this->assign('phone_authorize',$phone_authorize);
...
/*        $userInfo = unserialize($_SESSION[$this -> session_dish_user]);
    ...
    public function mymenu(){
        $userInfo = unserialize($_SESSION[$this -> session_dish_user]);
        if (empty($userInfo)){
            $this -> redirect(U('Repast/selectTable', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'cid' => $this -> _cid)));
        }
        $menu = $this -> getDishMenu();
        $data = array();
...
//打印方法 $dishCompany['memberCode'] && $dishCompany['feiyin_key'] && $dishCompany['deviceNo']
    ...
    public function payReturn(){
        ...
        if ($order = M('dish_order') -> where(array('orderid' => $orderid, 'token' => $this -> token)) -> find()){
            if ($order['paid']){
                Sms :: sendSms($this -> token . "_" . $this -> _cid, "顾客{$order['name']}刚刚对订单号：{$orderid}的订单进行了支付，请您注意查看并处理【赛微网络】");
            }
