...
class YiliaoAction extends BaseAction{
	...
	$info=M('Wxuser')->where(array('token'=>$this->token))->find();
        ...
        $yiliaostatus=$info['yiliaostatus'];
		$content = $this->sms();
		
		$phone=$info['phone'];
        $user=$info['smsuser'];//短信平台帐号
        $pass=md5($info['smspassword']);//短信平台密码
        $yiliaosmsstatus=$info['yiliaosmsstatus'];//短信平台状态
        $contentt = $this->smss();
		if(IS_POST){
	...
	public function smss(){
		...
		$orders=$this->yuyue_order->where(array('token'=>$this->token,'type'=>$this->type))->order('date desc')->limit(0,1)->find();
			...
			$str="亲爱的微最强网商户：以下是客户在你微最强平台下的新医疗预约订单哦：订单类型：".$orders['kind']."姓名：".$orders['name']."电话：".$orders['phone']."预约日期：".$orders['or_date']."预约时间：".$orders['time']."下单时间：".$orders['date'];
			//
