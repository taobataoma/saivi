...
class ShipinAction extends BaseAction{
	...
	public function index(){
		...
		$lp_zjdp=M('Shipincom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$info['title']))->order('sort desc')->select();
	...
	public function subestate_index(){
		...
		$info=M('Shipincom')->where(array('token'=>$token))->find();
		$lp_zjdp=M('Shipincom')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
		$this->assign('info',$info);
	...
	public function comments_all(){
		...
		$info=M('Shipincom')->where(array('token'=>$token))->find();
		$lp_zjdpall=M('Shipincom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$this->_get('subestatename')))->order('sort desc')->select();
		$this->assign('info',$info);
	...
	public function poster(){
		...
		$info=M('Shipinposter')->where(array('token'=>$token))->find();
		$lp_zjdp=M('Shipinposter')->where(array('token'=>$token,'subestatename'=>$this->_get('subestatename')))->order('id desc')->select();
		$this->assign('info',$info);
		$this->assign('zjdp',$lp_zjdp);
		$this->display();
	}
		public function Shipinphoto(){
		$agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")&&!isset($_SESSION['token'])) {
			echo '此功能只能在微信浏览器中使用';exit;
		}
		...
		$info=M('Shipinphoto')->where(array('token'=>$token))->find();
		$xc=M('Shipinphoto')->where(array('token'=>$token,'status'=>1))->order('id desc')->select();
		$toppic=M('Shipin')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
		...
		public function Shipinphoto_list(){
		$agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")&&!isset($_SESSION['token'])) {
			echo '此功能只能在微信浏览器中使用';exit;
		}
		...
		$info=M('Shipinphoto')->where(array('token'=>$token))->find();
		$xc=M('Shipinphoto')->where(array('token'=>$token,'status'=>1,'name'=>$this->_get('name')))->order('id desc')->select();
		$toppic=M('Shipin')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
	...
	public function photo(){
		...
		$info=M('Shipinphoto')->where(array('token'=>$token))->find();
		$xc=M('Shipinphoto')->where(array('token'=>$token,'status'=>1,'namephoto'=>$this->_get('namephoto'),))->order('sort desc')->select();
		$toppic=M('Shipin')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
		...
		$info=M('Shipinphoto')->where(array('token'=>$token))->find();
		$xc=M('Shipinphoto')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
		$toppic=M('Shipin')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
