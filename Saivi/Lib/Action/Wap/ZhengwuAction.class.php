...
class ZhengwuAction extends BaseAction{
	...
	public function index(){
		...
		$lp_zjdp=M('Zhengwucom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$info['title']))->order('sort desc')->select();
	...
	public function subestate_index(){
		...
		$lp_zjdp=M('Zhengwucom')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
	...
	public function comments_all(){
		...
		$lp_zjdpall=M('Zhengwucom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$this->_get('subestatename')))->order('sort desc')->select();
	...
	public function poster(){
		...
		$lp_zjdp=M('Zhengwuposter')->where(array('token'=>$token,'subestatename'=>$this->_get('subestatename')))->order('id desc')->select();
		$this->assign('info',$info);
		$this->assign('zjdp',$lp_zjdp);
		$this->display();
	}
		...
		$info=M('Zhengwuphoto')->where(array('token'=>$token))->find();
		$xc=M('Zhengwuphoto')->where(array('token'=>$token,'status'=>1))->order('id desc')->select();
		$toppic=M('Zhengwu')->where(array('token'=>$token))->find();
		...
		public function Zhengwuphoto_list(){
		$agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")&&!isset($_SESSION['token'])) {
			echo '此功能只能在微信浏览器中使用';exit;
		}
		...
		$info=M('Zhengwuphoto')->where(array('token'=>$token))->find();
		$xc=M('Zhengwuphoto')->where(array('token'=>$token,'status'=>1,'name'=>$this->_get('name')))->order('id desc')->select();
		$toppic=M('Zhengwu')->where(array('token'=>$token))->find();
	...
	public function photo(){
		...
		$info=M('Zhengwuphoto')->where(array('token'=>$token))->find();
		$xc=M('Zhengwuphoto')->where(array('token'=>$token,'status'=>1,'namephoto'=>$this->_get('namephoto'),))->order('id desc')->select();
		$toppic=M('Zhengwu')->where(array('token'=>$token))->find();
		...
		$info=M('Zhengwuphoto')->where(array('token'=>$token))->find();
		$xc=M('Zhengwuphoto')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
		$toppic=M('Zhengwu')->where(array('token'=>$token))->find();
