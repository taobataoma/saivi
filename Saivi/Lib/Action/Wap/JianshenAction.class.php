...
class JianshenAction extends BaseAction{
	...
	public function index(){
		...
		$lp_zjdp=M('Jianshencom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$info['title']))->order('sort desc')->select();
	...
	public function jianjie(){
		...
		$info=M('Jianshencom')->where(array('token'=>$token))->find();
		$lp_zjdp=M('Jianshencom')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
		$this->assign('info',$info);
	...
	public function comments_all(){
		...
		$info=M('Jianshencom')->where(array('token'=>$token))->find();
		$lp_zjdpall=M('Jianshencom')->where(array('token'=>$token,'status'=>1,'subestatename'=>$this->_get('subestatename')))->order('sort desc')->select();
		$this->assign('info',$info);
	...
	public function poster(){
		...
		$info=M('Jianshenposter')->where(array('token'=>$token))->find();
		$lp_zjdp=M('Jianshenposter')->where(array('token'=>$token,'subestatename'=>$this->_get('subestatename')))->order('id desc')->select();
		$this->assign('info',$info);
		$this->assign('zjdp',$lp_zjdp);
		$this->display();
	}
		public function Jianshenphoto(){
		$agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")&&!isset($_SESSION['token'])) {
			echo '此功能只能在微信浏览器中使用';exit;
		}
		...
		$info=M('Jianshenphoto')->where(array('token'=>$token))->find();
		$xc=M('Jianshenphoto')->where(array('token'=>$token,'status'=>1))->order('id desc')->select();
		$toppic=M('Jianshen')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
		...
		public function Jianshenphoto_list(){
		$agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")&&!isset($_SESSION['token'])) {
			echo '此功能只能在微信浏览器中使用';exit;
		}
		...
		$info=M('Jianshenphoto')->where(array('token'=>$token))->find();
		$xc=M('Jianshenphoto')->where(array('token'=>$token,'status'=>1,'name'=>$this->_get('name')))->order('id desc')->select();
		$toppic=M('Jianshen')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
	...
	public function photo(){
		...
		$info=M('Jianshenphoto')->where(array('token'=>$token))->find();
		$xc=M('Jianshenphoto')->where(array('token'=>$token,'status'=>1,'namephoto'=>$this->_get('namephoto'),))->order('sort desc')->select();
		$toppic=M('Jianshen')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
		...
		$info=M('Jianshenphoto')->where(array('token'=>$token))->find();
		$xc=M('Jianshenphoto')->where(array('token'=>$token,'status'=>1,'id'=>$this->_get('id')))->order('id desc')->select();
		$toppic=M('Jianshen')->where(array('token'=>$token))->find();
		$this->assign('toppic',$toppic);
		$this->assign('info',$info);
		$this->assign('xc',$xc);
