...
class IndexAction extends ChatAction
{
    ...
    public function index()
    {
        ...
        $list  = $data->where(array(
            'token' => $_SESSION['token'],
            'uid' => session('userId')
        ))->order('enddate  desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		if(is_array($list)){
        foreach ($list as $key => $limen) {
            ...
            $userInfo                      = M('wechat_group_list')->order('id desc')->where(array(
                'openid' => $limen['openid']
            ))->find();
