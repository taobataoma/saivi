<?php
//web
class HuisuoAction extends UserAction{
	public $token;
	public $Yuyue_model;
	public $yuyue_order;
	public $Huisuo_photo;
	...
	public function infos(){
		...
		if(empty($check)){$this->error('请填写好会所管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
	...
	public function setcin(){
		$id=$this->_get('pid');
		$checkdata=$this->Yuyue_model->where(array('id'=>$id))->find();
		if(empty($checkdata)){$this->error('请填写好KTV管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$cin=M('yuyue_setcin');
	...
	public function photo_index(){
		...
		$count = $this->Huisuo_photo->where($where)->count();

		$Page       = new Page($count,20);

		$show       = $Page->show();

		$data = $this->Huisuo_photo->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('show_order desc')->select();
	...
	public function photo_add(){
		...
		$pid = $this->Yuyue_model->where(array('token'=>$this->token))->field('id')->find();
		...
		if(IS_POST){
			...
			if($this->Huisuo_photo->add($_POST)){

				$this->success('添加成功！',U('Huisuo/photo_index',array('token'=>$this->token)));
	...
	public function photo_set(){
		...
		$checkdata = $this->Huisuo_photo->where(array('id'=>$id))->find();

		if(empty($checkdata)||$checkdata['token']!=$this->token){

            $this->error("没有相应记录.您现在可以添加.",U('Huisuo/photo_add'));
		...
		if(IS_POST){ 
			...
			$check=$this->Huisuo_photo->where($where)->find();

			if($check==false)$this->error('非法操作');

			if($this->Huisuo_photo->create()){

				if($this->Huisuo_photo->where($where)->save($_POST)){

					$this->success('修改成功',U('Huisuo/photo_index',array('token'=>$this->token)));
				...
				$this->error($this->Huisuo_photo->getError());
	...
	public function photo_del(){
        ...
        if(IS_GET){                            
            ...
            $back = $this->Huisuo_photo->where($where)->delete();

			if($back==true){

				$this->success('操作成功',U('Huisuo/photo_index',array('token'=>$this->token)));

			}else{

				$this->error('服务器繁忙,请稍后再试',U('Huisuo/photo_index',array('token'=>$this->token)));
	...
	//订单设置
	 public function setinfo(){ 
		...
		if(empty($checkdata)){$this->error('请填写KTV管理页面在再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$_POST['token'] = $this->token;
		...
		if(empty($list[0][name])){
			$list[0][name]="选择会所包销";
			$list[0][value]="包销201|包销202|包销203|包销205|包销206|包销207|包销208";
		}
		//print_r($list);die;
		$this->assign('list',$list);
