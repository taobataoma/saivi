...
class Medical_setupAction extends UserAction{
	public function index(){	
		$token_open=M('token_open')->field('queryname')->where(array('token'=>$_SESSION['token']))->find();
		if(!strpos($token_open['queryname'],'Medical')){
        $this->error('您还未开启该模块的使用权,请到功能模块中添加',U('Medical_setup/index',array('token'=>$_SESSION['token'],'id'=>session('wxid'))));}
		$db=D('Medical_setup');
