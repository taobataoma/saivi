...
class RecipeAction extends UserAction{
    ...
    public function _initialize() {
        ...
        // $function=M('Function')->where(array('funname'=>'recipe'))->find();
                ...
                if(!$t_recipe->create()){
                    ...
                    if('edit'==$status && $id != ''){
                        ...
                        if($id=$t_recipe->data($_POST)->add()){
                            ...
                            M('Keyword')->add($data1);
                            $this->success('添加成功',U("Recipe/index",array('token'=>session('token'),'type'=>$_POST['type'])));exit;
                        }else{
    ...
    public function index_del(){
        ...
        $result = $t_recipe->where($find)->find();
         if($result){
            ...
            exit($this->error('非法操作,请稍候再试',U("Recipe/index",array('token'=>session('token'),'type'=>$type))));
         }
    }
