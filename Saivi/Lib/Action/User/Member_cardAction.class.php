...
class Member_cardAction extends UserAction{
	...
	*/
	...
	public function recharge(){
		$db = M('Userinfo');
		$uid = (int)$_GET['uid'];
		$uinfo = $db->where(array('token'=>$this->token,'id'=>$uid))->field('wechaname,truename,id')->find();
		if(IS_POST){
...
//导出积分，线下消费记录
	public function exportCardUseRecord(){
		...
		$arr = array(
			...
			array('en'=>'wecha_id','cn'=>'微信号openID'),
...
//删除记录
	public function payRecord_del(){
		...
		$pay_record = M('Member_card_pay_record');
		if($pay_record->where(array('token'=>$token,'id'=>$id))->delete()){
			$this->success('删除成功');
		}else{
			$this->error('删除失败');
		}
