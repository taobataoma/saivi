   ...
   // public $wecha_id;
    ...
    public function manage(){
       ...
       // $books = $t_reservebook->where($where)->select();
        ...
        $this->assign('count',$t_reservebook->where(array('token'=>session('token'),'rid'=>$rid,'type'=>$addtype))->count());
        $this->assign('ok_count',$t_reservebook->where(array('token'=>session('token'),'rid'=>$rid,'type'=>$addtype,'remate'=>'1'))->count());
        $this->assign('lose_count',$t_reservebook->where(array('token'=>session('token'),'rid'=>$rid,'type'=>$addtype,'remate'=>'2'))->count());
        $this->assign('call_count',$t_reservebook->where(array('token'=>session('token'),'rid'=>$rid,'type'=>$addtype,'remate'=>'0'))->count());
        $this->display();
    }
