...
class MarketAction extends UserAction {
    ...
    public function _initialize() {
        ...
        $market_id = $this->market_db->where(array(
            'token' => $this->token
        ))->getField('market_id');
        if (empty($market_id) && ACTION_NAME != 'index') {
            $this->error('设置商圈后才能使用', U('Market/index', array(
                'token' => $this->token
            )));
        } else {
            ...
            if ($this->market_db->create()) {
                if ($market) {
                    ...
                    $keyword_db->where(array(
                        'token' => $this->token,
                        'pid' => $this->_post('market_id', 'intval') ,
                        'module' => 'Market'
                    ))->save($keyword);
                    $this->success('修改成功', U('Market/index', array(
                        'token' => $this->token
                    )));
                } else {
                    ...
                    $this->success('设置成功', U('Market/index', array(
                        'token' => $this->token
                    )));
                }
            } else {
    ...
    public function _create_nav($id) {
        ...
        $url = array(
            '{siteUrl}/index.php?g=Wap&m=Market&a=tenant&token=' . $this->token . '&wecha_id={$wecha_id}',
            '{siteUrl}/index.php?g=Wap&m=Market&a=park&token=' . $this->token . '&wecha_id={$wecha_id}',
            '{siteUrl}/index.php?g=Wap&m=Market&a=intro&token=' . $this->token . '&wecha_id={$wecha_id}',
        ...
        $Classify = M('Classify')->where(array(
            'token' => $this->token,
            'path' => '0',
            'fid' => 0
        ))->limit(2)->select();
    ...
    public function _market_slide($market_id, $data, $handle = 'insert') {
        ...
        if ($handle == 'insert') {
            ...
            $slide_id = $slide_db->where(array(
                'market_id' => $market_id
            ))->getField('slide_id', true);
            $count = count($data['slide_url']);
            for ($i = 0; $i < $count; $i++) {
                $arr['slide_url'] = $data['slide_url'][$i];
                $arr['slide_title'] = $data['slide_title'][$i];
                $arr['slide_link'] = $data['slide_link'][$i];
                $slide_db->where(array(
                    'market_id' => $market_id,
                    'slide_id' => $slide_id[$i]
                ))->save($arr);
            }
        ...
        if (IS_POST) {
            if ($this->area_db->create()) {
                if ($area) {
                    ...
                    $this->success('修改成功', U('Market/area', array(
                        'token' => $this->token
                    )));
                } else {
                    $_POST['token'] = $this->token;
                    ...
                    $this->success('添加成功', U('Market/area', array(
                        'token' => $this->token
                    )));
                }
            } else {
    ...
    public function area_del() {
        ...
        if ($this->area_db->where($where)->delete()) {
            $this->success('删除成功', U('Market/area', array(
                'token' => $this->token
            )));
        }
    }
    ...
    public function park_set() {
        $where = array(
            'token' => $this->token,
            'market_id' => $this->market_id,
            'park_id' => $this->_get('park_id', 'intval')
        );
        ...
        if (IS_POST) {
            if ($this->park_db->create()) {
                if ($park) {
                    ...
                    $this->success('修改成功', U('Market/park', array(
                        'token' => $this->token
                    )));
                } else {
                    $_POST['token'] = $this->token;
                    ...
                    $this->success('添加成功', U('Market/park', array(
                        'token' => $this->token
                    )));
                }
            } else {
    ...
    public function park_del() {
        ...
        if ($this->park_db->where($where)->delete()) {
            $this->success('删除成功', U('Market/park', array(
                'token' => $this->token
            )));
        }
    }
        ...
        if (IS_POST) {
            if ($this->cate_db->create()) {
                if ($cate) {
                    ...
                    $this->success('修改成功', U('Market/cate', array(
                        'token' => $this->token
                    )));
                } else {
                    $_POST['token'] = $this->token;
                    ...
                    $this->success('添加成功', U('Market/cate', array(
                        'token' => $this->token
                    )));
                }
            } else {
    ...
    function cate_del() {
        ...
        if ($this->cate_db->where($where)->delete()) {
            $this->success('删除成功', U('Market/cate', array(
                'token' => $this->token
            )));
        }
    }
    public function tenant() {
        $cate_list = $this->cate_db->where(array(
            'token' => $this->token,
            'market_id' => $this->market_id,
            'is_show' => '1'
        ))->field('cate_name,cate_id')->order('sort desc')->select();
        $area_list = $this->area_db->where(array(
            'token' => $this->token,
            'market_id' => $this->market_id,
            'is_use' => '1'
        ))->field('area_name,area_id')->order('sort desc,add_time desc')->select();
        $where = array(
            'token' => $this->token,
            'market_id' => $this->market_id,
            'isbranch' => '1'
        ...
        foreach ($tenant as $key => $value) {
            $tenant[$key]['cate_name'] = $this->cate_db->where(array(
                'token' => $this->token,
                'cate_id' => $value['cate_id']
            ))->getField('cate_name');
            $tenant[$key]['area_name'] = $this->area_db->where(array(
                'token' => $this->token,
    ...
    public function tenant_set() {
        ...
        $this->assign('area_list', $area_list);
        $this->assign('cate_list', $cate_list);
        $tenant_info = $this->tenant_db->where(array(
            'token' => $this->token,
            'market_id' => $this->market_id,
