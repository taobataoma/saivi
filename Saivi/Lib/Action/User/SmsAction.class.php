...
class SmsAction extends UserAction{
	...
	function buy_post(){
		...
		if ($needFee<$moneyBalance){
			...
			$indent=array();
			$indent['id']=time();
			M('Indent')->data(array('uid'=>session('uid'),'month'=>0,'title'=>'money'.intval($_POST['count']).'条','uname'=>$this->user['username'],'gid'=>0,'create_time'=>time(),'indent_id'=>$indent['id'],'price'=>$spend,'status'=>1))->add();
			M('Users')->where(array('id'=>$this->user['id']))->setDec('money',intval($needFee));
