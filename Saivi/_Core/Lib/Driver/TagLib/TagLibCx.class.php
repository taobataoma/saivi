...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'Saiviif'        =>  array('attr'=>'where','level'=>2),
		...
		'Saivilist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _Saiviif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'Saiviif');
	...
	public function _Saivilist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'Saivilist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
